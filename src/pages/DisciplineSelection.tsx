...
function DisciplineSelection() {
    ...
    const [humanitiesSelected, setHumanitiesSelected] = useState({1: null, 5: null, 6: null});
    const [openHumanitiesDropdown, setOpenHumanitiesDropdown] = useState({1: false, 5: false, 6: false});

    const toggleDisciplineSelection = (id: number) =>
        setDisciplinesSelected
        ((selected) => (selected.includes(id) ? selected.filter((discipline) => discipline !== id) : [...selected, id]));

    const toggleHumanitiesDropdown = (quarter: number) =>
        setOpenHumanitiesDropdown((selected) => ({...selected, [quarter]: !selected[quarter]}));

    const handleHumanitiesSelection = (quarter: number, courseId: number | null) =>
    ...
    const filteredDisciplines = disciplines.filter(
        (discipline) => discipline.quarter && discipline.courseCategory?.some((category) => [
    ...
    const groupedDisciplines = filteredDisciplines.reduce<Record<number, Discipline[]>>(
        (acc, discipline) => {
            ...
            <main className="container flex flex-col flex-1 mx-auto px-4 py-8">
                <div className="bg-white rounded-xl shadow-xs w-full border border-gray-200 p-4">
                    ...
                    <div className="grid grid-cols-1 gap-8 mb-8">
                        {Object.entries(groupedDisciplines).map(([quarter, disciplines]) => {
                                const q = parseInt(quarter);
                                const isHumanitiesQuarter = q === 1 || q === 5 || q === 6;

                                return (
                                    <div key={quarter}>
                                        <h3 className="text-lg font-semibold mb-4">{q}º Quadrimestre</h3>
                                        <div className="grid grid-cols-4 gap-4">
                                            {isHumanitiesQuarter && (
                                                <CardHumanidade
                                                    ...
                                                    handleSelect={(disciplineId) => (handleHumanitiesSelection(q, disciplineId))}
                                                />
                                            )}
                                            {disciplines.map((discipline) => (
                                                <CardObrigatoria
                                                    ...
                                                    toggleDiscipline={() => (toggleDisciplineSelection(discipline.id))}
                                                />
                                            ))}
                                            {q === 11 && <CardOptativa/>}
                                        </div>
                    ...
                    <div className="w-full flex justify-center">
                        <Button label="Confirmar seleção" className="bg-green-800 text-white text-sm">
